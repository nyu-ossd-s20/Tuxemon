...
class RenameMonsterAction(EventAction):
    ...
    def start(self):
        ...
        menu = self.session.control.push_state("MonsterMenuState")
        menu.on_menu_selection = self.prompt_for_name

    def update(self):
        if self.session.control.get_state_name("MonsterMenuState") is None and self.session.control.get_state_name("InputMenu") is None:
            self.stop()
