...
class TransitionTeleportAction(EventAction):
    ...
    **Examples:**
    ...
    def start(self):
        ...
        self.transition = self.session.control.event_engine.get_action("screen_transition", params)
    ...
    def update(self):
        ...
        if self.transition.done:
            ...
            self.session.control.event_engine.execute_action("delayed_teleport", self.raw_parameters[:-1])
            self.stop()
